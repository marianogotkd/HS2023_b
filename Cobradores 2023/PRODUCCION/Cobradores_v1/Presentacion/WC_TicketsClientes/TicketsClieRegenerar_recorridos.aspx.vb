Public Class TicketsClieRegenerar_recorridos
...
#End Region
  ...
  Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    If Not IsPostBack Then
      ...
      Dim ds_parametro As DataSet = DAparametro.Parametro_consultar_fecha(Session("Fecha_regenerar"))
          ...
          While i < ds_permisos.Tables(0).Rows.Count
            ...
            If (Menu = "G" And Opcion = "") Or (Menu = "G" And Opcion = "1") Then
              ...
              If (SubOpcion = "") Or (SubOpcion = "3") Then
                If (SSOpcion = "" Or (SSOpcion = "2")) Then
                  valido = "si"
                  '/////GRUPOS////////
...
#End Region
      ...
      Dim ds_ctacteq As DataSet = DAtickets.CtaCte_MovimientosBuscar2(CDate(HF_fecha.Value), CInt(Txt_DesdeRecorrido.Text), CInt(Txt_DesdeOrden.Text), CInt(Txt_HastaRecorridoCodigo.Text), CInt(Txt_HastaOrden.Text))
      ...
      If ds_ctacte.Tables("Consulta_J2a").Rows.Count <> 0 Then
        ...
        DS_ticketsclientes.Tables("Recorrido_orden").Rows.Clear()

        While indice < ds_ctacte.Tables("Consulta_J2a").Rows.Count
          Dim Recorrido As String = CStr(ds_ctacte.Tables("Consulta_J2a").Rows(indice).Item("R"))
          Dim Orden As String = CStr(ds_ctacte.Tables("Consulta_J2a").Rows(indice).Item("O"))

          Dim existe As String = "no"
          Dim ii As Integer = 0
          While ii < DS_ticketsclientes.Tables("Recorrido_orden").Rows.Count
            If (Recorrido = DS_ticketsclientes.Tables("Recorrido_orden").Rows(ii).Item("Recorrido")) And (Orden = DS_ticketsclientes.Tables("Recorrido_orden").Rows(ii).Item("Orden")) Then
              existe = "si"
              Exit While
            End If
            ii = ii + 1
          End While
          If existe = "no" Then
            DS_ticketsclientes.Tables("Cliente_CtacteInfo").Rows.Clear()
            DS_ticketsclientes.Tables("Cliente_CtacteInfo_2").Rows.Clear()
            DS_ticketsclientes.Tables("Cliente_PremiosInfo").Rows.Clear()
            'voy a una subrutina y agrego todos los clientes que tengan mismo recorrido y orden
            Dim jj As Integer = 0
            While jj < ds_ctacte.Tables("Consulta_J2a").Rows.Count
              If (Recorrido = CStr(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("R"))) And (Orden = CStr(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("O"))) Then
                Dim fila As DataRow = DS_ticketsclientes.Tables("Cliente_CtacteInfo").NewRow
                fila("Grupo_id") = CInt(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Grupo_id"))
                fila("Grupo_codigo") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Grupo_codigo")
                fila("Grupo_nombre") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Grupo_nombre")
                fila("Cliente") = CInt(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Cliente"))
                fila("Cliente_codigo") = CInt(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Cliente_codigo"))
                fila("Cliente_nombre") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Cliente_Nombre")
                fila("R") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("R")
                fila("O") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("O")
                fila("Recaudacion") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Recaudacion")
                fila("Comision") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Comision")
                fila("Premios") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Premios")
                fila("Reclamos") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Reclamos")
                fila("DejoGano") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGano")
                If CDec(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGano")) > 0 Then
                  fila("DejoGano_desc") = "DEJO:"
                Else
                  fila("DejoGano_desc") = "GANO:"
                End If
                fila("RecaudacionSC") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("RecaudacionSC")
                fila("ComisionSC") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("ComisionSC")
                fila("PremiosSC") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("PremiosSC")
                fila("ReclamosSC") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("ReclamosSC")
                fila("DejoGanoSC") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGanoSC")
                If CDec(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGanoSC")) > 0 Then
                  fila("DejoGanoSC_desc") = "DEJO:"
                Else
                  fila("DejoGanoSC_desc") = "GANO:"
                End If
                Dim DejoGano_sum As Decimal = CDec(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGano")) + CDec(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGanoSC"))
                ...
                End If
                fila("RecaudacionB") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("RecaudacionB")
                fila("ComisionB") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("ComisionB")
                fila("PremiosB") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("PremiosB")
                fila("ReclamosB") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("ReclamosB")
                fila("DejoGanoB") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGanoB")
                If CDec(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGanoB")) > 0 Then
                  fila("DejoGanoB_desc") = "DEJO:"
                Else
                  fila("DejoGanoB_desc") = "GANO:"
                End If
                DejoGano_sum = DejoGano_sum + CDec(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("DejoGanoB"))
                ...
                End If
                'fila("Saldoanterior") = ds_ctacte.Tables(0).Rows(jj).Item("Clientes_Saldoanterior")
                fila("Saldoanterior") = ds_ctacte.Tables("Consulta_J2a").Rows(indice).Item("CtaCte_SaldoAnterior")
                fila("Cobros") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Cobros")
                fila("Regalos") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Regalos")
                fila("Pagos") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Pagos")
                fila("Prestamo") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Prestamo")
                fila("CobPrestamo") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("CobPrestamo")
                fila("Credito") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Credito")
                fila("CobCredito") = ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("CobCredito")

                Dim ds_credi As DataSet = DAtickets.CobroCreditos_ClienteObtener(CDate(HF_fecha.Value), CInt(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Cliente")))
                ...
                'Dim Calculo As Decimal = DejoGano_sum + CtaCte_SaldoAnterior + PAGO + PagoRegalo + DI + EntregaPrestamo + DevolucionPrestamo + EntregaPrestamo + CreditoCuota
                ...
                Try
                  'Dim Clie_Saldo As Decimal = ds_ctacte.Tables(0).Rows(jj).Item("Clientes_Saldo")
                ...
                If (ds_ctacte.Tables("Consulta_J2a").Rows(indice).Item("Clientes_Imprime") = True) Or (Valido_Regalo = "si") Then
                  'Dim calculo_importe As Decimal = (100 * CDec(ds_ctacte.Tables(0).Rows(jj).Item("Clientes_SaldoRegalo"))) / CDec(ds_ctacte.Tables(0).Rows(jj).Item("Clientes_Regalo"))
                  ...
                  Try
                    ...
                    'calculo_importe = (100 * CDec(ds_ctacte.Tables("Consulta_J2a").Rows(indice).Item("Regalos"))) / CDec(ds_ctacte.Tables("Consulta_J2a").Rows(indice).Item("Clientes_Regalo"))
                  Catch ex As Exception
                  End Try
                  ...
                  If calculo_importe > 0 Then
                    fila("Regalo_desc") = "REGALO EN CONTRA % " + CStr(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Clientes_Regalo")) + ":"
                  Else
                    ...
                    'fila("Regalo_desc") = "REGALO A FAVOR % " + CStr(ds_ctacte.Tables(0).Rows(jj).Item("Clientes_Regalo")) + ":"
                  End If
                ...
                If reg_valido = "si" Then
                  ...
                  Dim DS_Premios As DataSet = DAtickets.Premios_Cliente_Obtener(CDate(HF_fecha.Value), CInt(HF_dia_id.Value), CInt(ds_ctacte.Tables("Consulta_J2a").Rows(jj).Item("Cliente")))
            ...
            If DS_ticketsclientes.Tables("Cliente_CtacteInfo").Rows.Count <> 0 Then
              ...
              Dim recorrido_dig As String = CStr(CInt(Recorrido))
              While recorrido_dig.Length < recorrido_longitud
                recorrido_dig = "0" + recorrido_dig
              End While
              Dim orden_dig As String = CStr(CInt(Orden))
              ...
              '---------------------------------------------------------------------------------------------------
              'agrego registro a recorrido_orden
              Dim fila_aux As DataRow = DS_ticketsclientes.Tables("Recorrido_orden").NewRow
              fila_aux("Recorrido") = Recorrido
              fila_aux("Orden") = Orden
              DS_ticketsclientes.Tables("Recorrido_orden").Rows.Add(fila_aux)
            End If
