...
#Region "Metodos"
  ...
  Private Sub carga_liquidacion_parcial(ByVal Datatable_RecorridosSeleccionados As DataTable, ByVal dtTemp As DataTable)
    ...
    Dim rows() As DataRow = PreOrd.Select("Cliente <> 0", "Cliente, T ASC")
  ...
  Private Sub carga_liquidacion_parcial_optimizada(ByVal dtTemp As DataTable)

    Dim ds_recorridos As DataSet = DALiquidacion.Liquidacion_validar_recorridosB(HF_fecha.Value)
    ...
    While i < dtTemp.Rows.Count
      ...
      If (XCargas_Suc = 0 Or XCargas_Suc = 1) And XCargas_R = False Then
        ...
        Dim filtro As String = "Codigo = " + "'" + CStr(XCargas_recorridocodigo) + "'"
        Dim rows_recorridos() As DataRow = ds_recorridos.Tables(0).Select(filtro)

        'Dim ds_recorridos As DataSet = DALiquidacion.Liquidacion_validar_recorridos(HF_fecha.Value, XCargas_recorridocodigo)
        Dim referencia_recorrido As String = rows_recorridos(0).Item("Referencia").ToString.ToUpper
        Dim Puntos_P1 = ds_recorridos.Tables(0).Rows(0).Item("P1")
  ...
  Private Sub btn_avanzar_ServerClick(sender As Object, e As EventArgs) Handles btn_avanzar.ServerClick
