Public Class Grupos_alta_b
    ...
    If Not IsPostBack Then
      ...
      If Session("grupos_op") = "modificar" Then
        ...
        If ds_info.Tables(0).Rows.Count <> 0 Then
          ...
          Try
            Txt_clientedeuda.Text = ds_info.Tables(0).Rows(0).Item("Clientedeuda")
          Catch ex As Exception
            Txt_clientedeuda.Text = 0
          End Try
  ...
  Private Sub lb_errores_blanqueo()
    ...
    lb_error_clieporcentaje.Text = "* El cliente no existe"
    lb_error_cliedeuda.Visible = False
    lb_error_cliedeuda.Text = "* El cliente no existe"
  ...
  Private Sub BOTON_GRABAR_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles BOTON_GRABAR.ServerClick
    ...
    Try
      ...
      Try
        ...
        If clieporcentaje <> 0 Then
          ...
          If ds_clie.Tables(0).Rows.Count = 0 Then
            ...
            valido_ingreso = "no"
          Else
            'Dim Grupo_id As Integer = ds_clie.Tables(0).Rows(0).Item("Grupo_id")
            ''buscar el codigo y validar que sea del mismo grupo
      ...
      Dim cliedeuda As Integer
      Try
        cliedeuda = CInt(Txt_clientedeuda.Text)
        If cliedeuda <> 0 Then
          'lo busco y valido
          Dim ds_clie As DataSet = DAclientes.Clientes_buscar_codigo(cliedeuda)
          If ds_clie.Tables(0).Rows.Count = 0 Then
            'aqui msj error.
            lb_error_cliedeuda.Visible = True
            valido_ingreso = "no"
          End If
        End If
      Catch ex As Exception
        ...
        Select Case Session("grupos_op")
          Case "alta"
            If Session("grupos_op") = "alta" Then
              ...
              If (ds_info.Tables(1).Rows.Count = 0) Then 'no existe
                Dim valido_alta As String = "si"
                If clieporcentaje <> 0 Then
                  '1a) verifico que el cliente porcentaje "ingresado" no esté asignado como "cliente porcentaje" a otro grupo
                  Dim ds_consulta1 As DataSet = DAgrupos.Grupo_buscarClientePorcentaje(CStr(clieporcentaje))
                  If ds_consulta1.Tables(0).Rows.Count <> 0 Then
                    valido_alta = "no"
                    lb_error_clieporcentaje.Text = "* Error, ya fue asignado."
                    lb_error_clieporcentaje.Visible = True
                  End If
                End If
                If cliedeuda <> 0 Then
                  '1b) verifico que el cliente deuda "ingresado" no esté asignado como "cliente porcentaje" a otro grupo
                  Dim ds_consulta2 As DataSet = DAgrupos.Grupo_buscarClienteDeuda(CStr(cliedeuda))
                  If ds_consulta2.Tables(0).Rows.Count <> 0 Then
                    valido_alta = "no"
                    lb_error_cliedeuda.Text = "* Error, ya fue asignado."
                    lb_error_cliedeuda.Visible = True
                  End If
                End If
                If valido_alta = "si" Then
                  '2) guardo en bd
                  DAgrupos.Grupos_alta(Txt_grupo_nomb.Text, CStr(DropDownList_tipo.SelectedValue), porcentaje, clieporcentaje, CStr(DropDownList_codcobro.SelectedValue), fecha, CDec(0), CDec(0), CDec(0), Txt_grupo_codigo.Text, importe_procesamiento, cliedeuda)
                  Limpiar_campos()
          ...
          Case "modificar"
            If Session("grupos_op") = "modificar" Then
              ...
              If existe = "no" Then
                Dim valido_modif As String = "si"
                'a) verifico que el cliente porcentaje "ingresado" no esté asignado como "cliente porcentaje" a otro grupo
                If clieporcentaje <> 0 Then
                  Dim ds_consulta1 As DataSet = DAgrupos.Grupo_buscarClientePorcentaje(CStr(clieporcentaje))
                  If ds_consulta1.Tables(0).Rows.Count <> 0 Then
                    If (Txt_grupo_codigo.Text <> CStr(ds_consulta1.Tables(0).Rows(0).Item("Codigo"))) Then
                      valido_modif = "no"
                      lb_error_clieporcentaje.Text = "* Error, ya fue asignado."
                      lb_error_clieporcentaje.Visible = True
                    End If
                ...
                If cliedeuda <> 0 Then
                  ...
                  Dim ds_consulta2 As DataSet = DAgrupos.Grupo_buscarClienteDeuda(CStr(cliedeuda))
                  If ds_consulta2.Tables(0).Rows.Count <> 0 Then
                    If CStr(ds_consulta2.Tables(0).Rows(0).Item("Codigo")) <> Txt_grupo_codigo.Text Then
                      valido_modif = "no"
                      lb_error_cliedeuda.Text = "* Error, ya fue asignado."
                      lb_error_cliedeuda.Visible = True
                    End If
                  End If
                End If
                If valido_modif = "si" Then
                  DAgrupos.Grupos_modificar(CInt(HF_grupo_id.Value), Txt_grupo_nomb.Text, CStr(DropDownList_tipo.SelectedValue), porcentaje, clieporcentaje, CStr(DropDownList_codcobro.SelectedValue), fecha, Txt_grupo_codigo.Text, importe_procesamiento, cliedeuda)
                  Limpiar_campos()
  ...
  End Sub
  Private Sub Txt_clientedeuda_Init(sender As Object, e As EventArgs) Handles Txt_clientedeuda.Init
    Txt_clieporcentaje.Attributes.Add("onfocus", "seleccionarTexto(this);")
  End Sub
