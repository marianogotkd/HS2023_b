Public Class Cob_LocalAlta
  ...
  Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    If Not IsPostBack Then
      ...
      Session("AUX_TARIFAMODIF") = DS_Cob_local.Tables("AUX_TARIFAMODIF")
  ...
  Private Sub GridView1_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GridView1.RowCommand
    ...
    Else
      If (e.CommandName = "ID1") Then 'modificar precio
        GridView1.Columns(0).Visible = True 'columna TARIFA_ID
        GridView1.Columns(2).Visible = True 'columna LOCAL_ID
        Dim j As Integer = 0
        ...
        While j < GridView1.Rows.Count
          If GridView1.Rows(j).Cells(1).Text = rowselect_descripcion Then
            precio = GridView1.Rows(j).Cells(4).Text
            TARIFA = GridView1.Rows(j).Cells(1).Text
            Exit While
        ...
        HF_TARIFA_itemdesc.value = TARIFA
        txt_modif_precio.Text = CStr(precio).ToUpper.Replace(",", ".")

        ScriptManager.RegisterStartupScript(Page, Page.[GetType](), "Mdl_modificarprecio", "$(document).ready(function () {$('#Mdl_modificarprecio').modal();});", True)
    ...
    If HF_LOCAL_ID.Value <> 0 Then
      ...
      While i < GridView1.Rows.Count
        ...
        Else
          ...
          'Session("AUX_TARIFAMODIF") = DS_Cob_local.Tables("AUX_TARIFAMODIF")
          DS_Cob_local.Tables("AUX_TARIFAMODIF").Rows.Clear()
          DS_Cob_local.Tables("AUX_TARIFAMODIF").Merge(Session("AUX_TARIFAMODIF"))

          Dim j As Integer = 0
          While j < DS_Cob_local.Tables("AUX_TARIFAMODIF").Rows.Count
            If TARIFA_ID = DS_Cob_local.Tables("AUX_TARIFAMODIF").Rows(j).Item("TARIFA_ID") Then
              'modifico en tabla tarifa, solo necesito modificar el precio y busco por TARIFA_ID

              'update Tarifa set TARIFA_precio = '' where Tarifa.TARIFA_ID = ''
              Dim PRECIO As String = GridView1.Rows(i).Cells(4).Text.ToUpper
              PRECIO = PRECIO.ToUpper.Replace(",", ".")

              daTarifa.Tarifa_modificar(CStr(TARIFA_ID), PRECIO)

              daTarifa.TarifaCliente_modificar(CStr(TARIFA_ID), PRECIO, PRECIO)
  ...
  Private Sub Btn_modifprecio_confirmar_ServerClick(sender As Object, e As EventArgs) Handles Btn_modifprecio_confirmar.ServerClick
    ...
    txt_modif_precio.Text = txt_modif_precio.Text.ToUpper.Replace(".", ",")

    Try
      txt_modif_precio.Text = CDec(txt_modif_precio.Text)
    Catch ex As Exception
      valido = "no"
    End Try
    ...
    If valido = "si" Then
      ...
      Dim precio As Decimal = 0
      Dim TARIFA As String = ""
      Dim rowselect_descripcion As String = HF_TARIFA_itemdesc.Value

      While j < GridView1.Rows.Count
        If GridView1.Rows(j).Cells(1).Text = rowselect_descripcion Then
          GridView1.Rows(j).Cells(4).Text = txt_modif_precio.Text

          If GridView1.Rows(j).Cells(0).Text <> 0 Then

            'Session("AUX_TARIFAMODIF") = DS_Cob_local.Tables("AUX_TARIFAMODIF")
            DS_Cob_local.Tables("AUX_TARIFAMODIF").Merge(Session("AUX_TARIFAMODIF"))

            Dim fila As DataRow = DS_Cob_local.Tables("AUX_TARIFAMODIF").NewRow
            fila("TARIFA_ID") = CInt(GridView1.Rows(j).Cells(0).Text)
            DS_Cob_local.Tables("AUX_TARIFAMODIF").Rows.Add(fila)
    ...
    HF_TARIFA_itemdesc.Value = ""
    txt_modif_precio.Text = ""
