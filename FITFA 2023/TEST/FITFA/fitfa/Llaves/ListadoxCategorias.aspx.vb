Public Class ListadoxCategorias
...
#End Region
    ...
    Private Sub obtener_categorias(ByVal evento_id As Integer)
        ...
        Dim ds_categorias As DataSet = DAllave.LLave_obtener_inscriptos_filtrados2(CStr(evento_id), CStr(DropDown_modalidad.SelectedValue))
    ...
    Private Sub busqueda()
        ...
        If DropDown_categoria.Items.Count <> 0 Then
            Dim ds As DataSet = DAllave.LLave_obtener_inscriptos_sin_llave2(CStr(HF_evento_id.Value), CStr(DropDown_categoria.SelectedValue))
    ...
    Private Sub cargar_rpt(ByVal categoria_SEXO As String, ByVal edadinicial As String, ByVal ds_graduacion As DataSet, ByRef Llaves_ds As DataSet, ByVal orden As Integer)
        'ORDEN 1, categoria_id = 2, 327, 88, 328, 174, 329
        Dim ds_cat_o1 As DataSet = DAllave.obtener_categoria_x_orden(categoria_SEXO, edadinicial)
        Dim i As Integer = 0
        While i < ds_cat_o1.Tables(0).Rows.Count

            Dim categoria_id As Integer = ds_cat_o1.Tables(0).Rows(i).Item("categoria_id")
            Dim ds_inscriptos As DataSet = DAllave.LLave_obtener_inscriptos_sin_llave2(CStr(HF_evento_id.Value), CStr(categoria_id))
            Dim j As Integer = 0
            If ds_inscriptos.Tables(0).Rows.Count <> 0 Then
                'la categoria va concatenada en una var string
                Dim tipo As String = ds_cat_o1.Tables(0).Rows(i).Item("categoria_tipo")
                'busco graduacion desde
                Dim graduacion_desde As String = ""
                Dim k As Integer = 0
                While k < ds_graduacion.Tables(0).Rows.Count
                    If (ds_graduacion.Tables(0).Rows(k).Item("graduacion_id") = ds_cat_o1.Tables(0).Rows(i).Item("categoria_gradinicial")) Then
                        graduacion_desde = ds_graduacion.Tables(0).Rows(k).Item("graduacion_desc")
                        k = ds_graduacion.Tables(0).Rows.Count
                    End If
                    k = k + 1
                End While
                'busco graduacion hasta
                ...
                k = 0
                While k < ds_graduacion.Tables(0).Rows.Count
                    If ds_graduacion.Tables(0).Rows(k).Item("graduacion_id") = ds_cat_o1.Tables(0).Rows(i).Item("categoria_gradfinal") Then
                        graduacion_hasta = ds_graduacion.Tables(0).Rows(k).Item("graduacion_desc")
                        k = ds_graduacion.Tables(0).Rows.Count
                    End If
                    k = k + 1
                End While
                Dim edad_desde As String = ds_cat_o1.Tables(0).Rows(i).Item("categoria_edadinicial")
                Dim edad_hasta As String = ds_cat_o1.Tables(0).Rows(i).Item("categoria_edadfinal")
                Dim peso_inicial As String = ds_cat_o1.Tables(0).Rows(i).Item("categoria_peso_inical")
                Dim peso_final As String = ds_cat_o1.Tables(0).Rows(i).Item("categoria_peso_Final")
                Dim sexo As String = ds_cat_o1.Tables(0).Rows(i).Item("categoria_sexo")
                'ahora junto todas las variables para mostrar en categoria
                Dim categoria As String = tipo + " " + sexo + ", " + graduacion_desde + "-" + graduacion_hasta + "(" + edad_desde + " a " + edad_hasta + " años)"
                If tipo = "Lucha" Then
                    categoria = tipo + " " + sexo + ", " + graduacion_desde + "-" + graduacion_hasta + "(" + edad_desde + " a " + edad_hasta + " años)" + " de " + peso_inicial + " a " + peso_final + " Kilos"
                ...
                While j < ds_inscriptos.Tables(0).Rows.Count
                    Dim fila As DataRow = Llaves_ds.Tables("inscriptos_reporte").NewRow
                    fila("ID") = ds_inscriptos.Tables(0).Rows(j).Item("ID")
                    fila("dni") = ds_inscriptos.Tables(0).Rows(j).Item("dni")
                    fila("ApellidoyNombre") = ds_inscriptos.Tables(0).Rows(j).Item("ApellidoyNombre")
                    fila("Institucion_abreviatura") = ds_inscriptos.Tables(0).Rows(j).Item("Institucion_abreviatura")
                    fila("Institucion") = ds_inscriptos.Tables(0).Rows(j).Item("Institucion")
                    fila("Provincia") = ds_inscriptos.Tables(0).Rows(j).Item("Provincia")
                    fila("instructor_id") = ds_inscriptos.Tables(0).Rows(j).Item("instructor_id")
                    Dim instructor_id As Integer = ds_inscriptos.Tables(0).Rows(j).Item("instructor_id")
                    Dim ds_instr As DataSet = DAinstructor.Instructor_obtener_INFO(instructor_id)
                    fila("instructor") = ds_instr.Tables(0).Rows(0).Item("ApellidoyNombre")
                    fila("Peso") = ds_inscriptos.Tables(0).Rows(j).Item("Peso")
                    fila("Inscripcion_id") = ds_inscriptos.Tables(0).Rows(j).Item("inscripcion_id")
                    fila("orden") = orden
                    fila("torneo_id") = ds_inscriptos.Tables(0).Rows(j).Item("torneo_id")
                    fila("categoria_id") = categoria_id
                    fila("categoria_desc") = categoria
                    Llaves_ds.Tables("inscriptos_reporte").Rows.Add(fila)
                    j = j + 1
                End While
            End If
            i = i + 1
    ...
    Private Sub orden_obtener(ByRef orden As Integer)
        Dim ds_cat As DataSet = DAcategoria.Categoria_obtener_info(DropDown_categoria.SelectedValue)
        Dim sexo As String = ds_cat.Tables(0).Rows(0).Item("categoria_sexo")
        Dim EdadInicial As String = ds_cat.Tables(0).Rows(0).Item("categoria_edadinicial")

        If sexo = "AMBOS SEXOS" Then
            If EdadInicial = "0" Then
                orden = 1
            End If
            If EdadInicial = "6" Then
                orden = 2
            End If
            If EdadInicial = "8" Then
                orden = 3
            End If
            If EdadInicial = "10" Then
                orden = 4
            End If
        End If
        If sexo = "Mujer" Then
            If EdadInicial = "12" Then
                orden = 5
            End If
            If EdadInicial = "14" Then
                orden = 7
            End If
            If EdadInicial = "16" Then
                orden = 9
            End If
            If EdadInicial = "18" Then
                orden = 11
            End If
            If EdadInicial = "36" Then
                orden = 13
            End If
            If EdadInicial = "46" Then
                orden = 15
            End If
            If EdadInicial = "56" Then
                orden = 17
            End If
        End If
    ...
    Private Sub REPORTE_RECUPERAR_SELECCION()
        Dim ds_graduacion As DataSet = DAllave.obtener_graduacion

        Dim Llaves_ds As New Llaves_ds
        Llaves_ds.Tables("inscriptos_reporte").Rows.Clear()

        Dim categoria_id As Integer = DropDown_categoria.SelectedValue
        Dim ds_inscriptos As DataSet = DAllave.LLave_obtener_inscriptos_sin_llave2(CStr(HF_evento_id.Value), CStr(categoria_id))
        Dim J As Integer = 0
        While j < ds_inscriptos.Tables(0).Rows.Count
            Dim fila As DataRow = Llaves_ds.Tables("inscriptos_reporte").NewRow
            ...
            fila("torneo_id") = ds_inscriptos.Tables(0).Rows(j).Item("torneo_id")
            fila("categoria_id") = categoria_id
            fila("categoria_desc") = DropDown_categoria.SelectedItem.Text
            Llaves_ds.Tables("inscriptos_reporte").Rows.Add(fila)
            j = j + 1
        End While


        If Llaves_ds.Tables("inscriptos_reporte").Rows.Count <> 0 Then
            Dim fila As DataRow = Llaves_ds.Tables("inscriptos_info_reporte").NewRow
            fila("Evento") = Lb_evento.Text
            fila("Fecha") = CDate(Lb_fecha.Text)
            Llaves_ds.Tables("inscriptos_info_reporte").Rows.Add(fila)

            'aqui envio los datatable al reporte

            Session("datatable_inscriptos_reporte") = Llaves_ds.Tables("inscriptos_reporte")
            Session("datatble_inscriptos_info_reporte") = Llaves_ds.Tables("inscriptos_info_reporte")

            Response.Redirect("ListadoxCategorias_view.aspx")
    ...
    Private Sub REPORTE_RECUPERAR_TODO()
        Dim ds_graduacion As DataSet = DAllave.obtener_graduacion
        ...
        'ORDEN 1, categoria_id = 2, 327, 88, 328, 174, 329
        cargar_rpt("AMBOS SEXOS", "0", ds_graduacion, Llaves_ds, 1)

        'ORDEN 2, categoria_id = 3, 330, 89, 331, 175, 332
        cargar_rpt("AMBOS SEXOS", "6", ds_graduacion, Llaves_ds, 2)

        'ORDEN 3, categoria_id = 4, 333, 90, 334, 176, 335
        cargar_rpt("AMBOS SEXOS", "8", ds_graduacion, Llaves_ds, 3)

        'ORDEN 4, categoria_id = 5, 336, 91, 337, 177, 338, 262, 339, 340 
        cargar_rpt("AMBOS SEXOS", "10", ds_graduacion, Llaves_ds, 4)

        'cateogira mujer lucha forma 12-13 años
        'ORDEN 5
        cargar_rpt("Mujer", "12", ds_graduacion, Llaves_ds, 5)

        'categoria hombre lucha forma 13-13 años
        'orden 6
        cargar_rpt("Hombre", "12", ds_graduacion, Llaves_ds, 6)

        'CATEGORIA MUJER LUCHA FORMA 14-15 AÑOS
        'ORDEN 7
        cargar_rpt("Mujer", "14", ds_graduacion, Llaves_ds, 7)

        'categoria HOMBRE LUCHA FORMA 14-15 AÑOS
        'ORDEN 8
        cargar_rpt("Hombre", "14", ds_graduacion, Llaves_ds, 8)

        'categoria mujer lucha forma 16-17 años
        'ORDEN 9 ---
        cargar_rpt("Mujer", "16", ds_graduacion, Llaves_ds, 9)

        'CATEGORIA HOMBRE LUCHA FORMA 16-17 AÑOS
        'ORDEN 10----
        cargar_rpt("Hombre", "16", ds_graduacion, Llaves_ds, 10)

        'CATEGORIA MUJER LUCHA FORMA 18-35 AÑOS
        'ORDEN 11---
        cargar_rpt("Mujer", "18", ds_graduacion, Llaves_ds, 11)

        'CATEGORIA HOMBRE LUCHA FORMA 18-35 AÑOS
        'ORDEN 12
        cargar_rpt("Hombre", "18", ds_graduacion, Llaves_ds, 12)

        'CATEGORIA MUJER LUCHA FORMA 36-45 AÑOS
        'ORDEN 13
        cargar_rpt("Mujer", "36", ds_graduacion, Llaves_ds, 13)

        'CATEGORIA HOMBRE LUCHA FORMA 36 AÑOS
        'ORDEN 14
        cargar_rpt("Hombre", "36", ds_graduacion, Llaves_ds, 14)

        'CATEGORIA MUJER LUCHA FORMA 46 AÑOS
        'ORDEN 15
        cargar_rpt("Mujer", "46", ds_graduacion, Llaves_ds, 15)

        'CATEGORIA HOMBRE LUCHA FORMA 46 AÑOS
        'ORDEN 16
        cargar_rpt("Hombre", "46", ds_graduacion, Llaves_ds, 16)

        'CATEGORIA MUJER LUCHA FORMA 56 AÑOS
        'ORDEN 17
        cargar_rpt("Mujer", "56", ds_graduacion, Llaves_ds, 17)

        'CATEGORIA HOMBRE LUCHA FORMA 56 AÑOS
        'ORDEN 18
        cargar_rpt("Hombre", "56", ds_graduacion, Llaves_ds, 18)
    ...
    Private Sub Btn_rptTodo_Click(sender As Object, e As EventArgs) Handles Btn_rptTodo.Click
        REPORTE_RECUPERAR_TODO()
    End Sub
    ...
    Private Sub Btn_rptSelec_Click(sender As Object, e As EventArgs) Handles Btn_rptSelec.Click
        REPORTE_RECUPERAR_SELECCION()
